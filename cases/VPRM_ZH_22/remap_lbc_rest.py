...
def remap_lbc_rest(file):
    ...
    os.system('cdo setpartabn,/scratch/snx3000/nponomar/Emissions/mypartab_lbc,convert -selname,pres,temp,u,v,w,qv,qi,qr,qc,qs,fis,geopot,rho,theta_v,z_ifc,CO2_RA,CO2_GPP,TRCO2_BG_chemtr,TRCO2_Anthropogenic_chemtr ' + input_file_name + ' tmplbc'+os.path.basename(file)[:-8]+'.nc')
    os.system('cdo -s '+remapping_method+',triangular-grid_lbc.nc -selname,PS,T,U,V,W,QV,QI,QR,QC,QS,GEOSP,GEOP_ML,DEN,THETA_V,TRCO2_BG_RA,TRCO2_BG_GPP,TRCO2_BG,TRCO2_BG_A' + ' tmplbc'+os.path.basename(file)[:-8]+'.nc ' +  output_file_name)
